...
import com.magrathea.ppsimple.infra.adapters.inbound.rest.data.requests.TransferRequest
...
class TransferRestAdapter(
    ...
    fun doTransfer(
        ...
        val result = doTransferService.execute(transferRequest.toDoTransferUseCaseInput())
        val response = TransferResponse(externalId = result)

        return ResponseEntity.status(HttpStatus.CREATED).body(response)
    }

    private fun TransferRequest.toDoTransferUseCaseInput() = DoTransferUseCase.Input(
        payer = this.payer,
        payee = this.payee,
        value = this.value
    )
